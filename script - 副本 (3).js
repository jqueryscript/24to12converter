...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI() {
        ...
        if (is24to12) {
            ...
            timeInput.maxLength = 8; // e.g., "12:00 AM"
    ...
    function handleInput(e) {
        if (is24to12) {
            ...
            // For 12h input, we allow more characters, so we don't format as aggressively.
            // Validation will happen on conversion.
            if (timeInput.value.length >= 7) { // e.g., 1:00 AM
                 handleConversion();
            }
        }
    }
    ...
    function convert12to24(time12) {
        const originalInput = time12.toLowerCase();
        ...
        let [hours, minutes] = originalInput.replace(period, '').trim().split(':');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    themeToggleBtn.classList.add('theme-toggle-btn', 'fixed', 'bottom-4', 'right-4', 'p-2', 'rounded-full', 'bg-slate-200', 'dark:bg-slate-800', 'text-slate-600', 'dark:text-slate-300', 'hover:bg-slate-300', 'dark:hover:bg-slate-700', 'transition-colors', 'shadow-lg');
